...
require("ui.uieditor.widgets.BA.HUD.ThirdpersonCrosshair")
...
local function AddCustomHUDElements_Zombie(menu, controller)
    ...
    ZMNotificationContainer:subscribeToGlobalModel(controller, "PerController", "scriptNotify", function(model)
        ...
        if model and Engine.GetModelValue( model ) then
            ...
            if not string.match( param, "ba_" ) then
                return
            end
        else
            return
        ...
        if IsParamModelEqualToString(model, "ba_cheats_notification") then
            AddCheatsNotification(self, container, model)
        -- elseif IsParamModelEqualToString(model, "ba_mob_zombies_notification") then
        --     AddSimpleNotification(self, container, "t7_ba_notification_idk", "BA_ZM_NOTIFICATION_MOB_ZOMBIES", "BA_ZM_NOTIFICATION_MOB_ZOMBIES_DESC")
        end
    end)
