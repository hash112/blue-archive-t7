...
#define DEV_ONLY_XUID_ARRAY array( "110000112604b26", "" ) // Change this to your own XUID -- KEEP EMPTY FOR ONLY USERNAME -- CAN ADD SEVERAL (ARRAY USAGE)
...
/*##                    COMMAND RESPONSES                      ##*/
/*###############################################################*/
function private _print_perks()
{
    ModVar("print_perks", "");
    for(;;){
        WAIT_SERVER_FRAME;

        dvar_value = ToLower(GetDvarString("print_perks", ""));

        if(isdefined(dvar_value) && dvar_value != "")
        {
            ModVar("print_perks", "");

            foreach(perk in level.struct_class_names[ "targetname" ][ "zm_perk_machine" ])
            {
                IPrintLnBold(perk.model);
                IPrintLnBold(perk.script_noteworthy);
                wait 1;
            }
