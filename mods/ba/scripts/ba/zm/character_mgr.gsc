...
{
	util::registerClientSys( "ba_character_icon" );
	if( is_enabled() )
	{
...
{
    util::setClientSysState( "ba_character_icon", "none", self );
...
function zombie_model_override()
{
	...
	if ( !GetDvarInt("tfoption_mob_zombie", 0) )
	{
		self zombie_model_fix();
		return;
	}

	// modded zombie skeleton are not compatible with the default one used by ba assets
	...
	type = "generic_mob";
	switch( GetDvarInt("tfoption_mob_zombie", 0) )
	{
		case 1:
		{
			type = "generic_mob";
...
function set_icon(func_index)
{
	...
	util::setClientSysState( "ba_character_icon", icon_index, self );
}
